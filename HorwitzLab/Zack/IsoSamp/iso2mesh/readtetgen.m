...
%
% read node file
    fp = fopen([fstub '.node'], 'r');
    if fp == -1, error('node file is missing!'); end
    ...
    % read element file
    fp = fopen([fstub '.ele'], 'r');
    if fp == -1, error('elem file is missing!'); end
    ...
    if fp == -1, error('surface data file is missing!'); end
