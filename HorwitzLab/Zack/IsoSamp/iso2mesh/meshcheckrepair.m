...
%
    if nargin < 3 || strcmp(opt, 'duplicated')
        ...
        l2 = length(elem);
        if l2 ~= l1, fprintf('%d duplicated elements were removed\n', l1-l2); end
    end
    ...
    if nargin < 3 || strcmp(opt, 'isolated')
        ...
        l2 = length(node);
        if l2 ~= l1, fprintf('%d isolated nodes were removed\n', l1-l2); end
    end
    ...
    if nargin < 3 || strcmp(opt,'deep')
        ...
        system([' "' mcpath('meshfix') exesuff '" "' mwpath('pre_sclean.off') '" "' mwpath('post_sclean.off') '" > nul']);
