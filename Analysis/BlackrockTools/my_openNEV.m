function varargout = my_openNEV(varargin)
...
for i=1:length(varargin)
    switch lower(varargin{i})
        ...
        otherwise
            temp = varargin{i};
            if any(regexpi(temp, '.nev'))                
                fileFullPath = varargin{i};
                if exist(fileFullPath, 'file') ~= 2
                    error('The .nev filename does not exist in the pwd, or was not specified as a fully qualified path.');
                end
            ...
            for idx = 1:length(splitDigiValues)
                try
                    if isempty(find(splitDigiValues{idx} == ':', 1))
                        ...
                        [tempParsedCommand, tmperror] = parseCommand(splitDigiValues{idx});
                        if ~tmperror
