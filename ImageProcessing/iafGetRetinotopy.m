...
% store the raw data (separated into ON trials OFF trials, and by stimulus
% type
udat.raw_OnResponse = dat.OnResponse;
udat.raw_OffResponse = dat.OffResponse;
...
function updateImage(~,~)
    ...
    final_img = udat.final_img{imgNum};
...
function img_preProcess(~,~)
    ...
    for i_type = 1 : Nttypes
        udat.AvgTrialON{i_type} = iafGetAvgMovie(udat.filtData_ON{i_type});
        udat.AvgTrialOFF{i_type} = iafGetAvgMovie(udat.filtData_OFF{i_type});
